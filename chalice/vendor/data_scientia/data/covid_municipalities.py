...
def get(municipio_code, filter_just_municipio=False):
    ...
    ::
        ...
        data.iloc[0]
    """
    print("Getting "+str(municipio_code))
    s3_resource = boto3.resource('s3')
    bucket = 'data-scientia'
    csv_buffer = s3_resource.Object(bucket, municipio_code+'.csv').get()['Body'].read().decode('utf-8')
    data = pd.read_csv(StringIO(csv_buffer))
    data['municipio_code'] = data['municipio_code'].astype(str)
    data['Municipality ID'] = data['Municipality ID'].astype(str)
